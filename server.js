// 1. OBRIGATÓRIO: Carrega as variáveis de ambiente antes de qualquer coisa
...
// --- DEBUG DE VARIÁVEIS (Para checar no painel da Hostinger) ---
console.log("========================================");
console.log("🔍 CHECKUP INICIAL DO SERVIDOR:");
console.log("DB_HOST:", process.env.DB_HOST);
console.log("DB_USER:", process.env.DB_USER);
console.log("EMAIL_USER:", process.env.EMAIL_USER ? "OK (Carregado)" : "VAZIO (Atenção!)");
console.log("Stripe Key:", process.env.STRIPE_SECRET_KEY ? "OK (Carregada)" : "VAZIA");
console.log("========================================");

// 2. Importa o banco de dados (agora que as variáveis já foram carregadas)
const pool = require('./db'); 

// 3. Configurações Iniciais
const app = express();

// CORREÇÃO PARA HOSTINGER: Permite que o servidor entenda que está atrás de um proxy
...
// Configuração do Stripe (com proteção anti-crash)
...
// Segurança: Limitador de tentativas de login
...
// --- CONFIGURAÇÃO DE E-MAIL (NODEMAILER - MODO HOSTINGER) ---
const transporter = nodemailer.createTransport({
    ...
    tls: { rejectUnauthorized: false } // Ajuda a evitar erros de certificado
});
...
// Rota: Configuração pública (Chave pública do MP)
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        // Se existe mas está pendente, limpa para tentar de novo
...
// Rota: Cancelar Cadastro (Limpar dados)
...
// --- RECUPERAÇÃO DE SENHA (NODEMAILER) ---
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        ...
        // GERA O LINK (Usa o domínio atual)
        ...
        const mailOptions = {
            from: `"Suporte Próxyz" <${process.env.EMAIL_USER}>`,
            to: email,
            subject: 'Redefinição de Senha',
            html: htmlContent
        };

        console.log("4. Enviando e-mail...");
        await transporter.sendMail(mailOptions);
        console.log("5. E-mail enviado!");
...
// Rota: Pagamento via BRICK (Cartão/Débito)
    ...
    if (type === 'payment') {
        try {
            ...
            if (info.status === 'approved') {
                ...
                if (userId) {
                    console.log(`Webhook: Aprovado para User ${userId}`);
                    await pool.query('UPDATE users SET status = "active" WHERE id = ?', [userId]);
                    await pool.query(
